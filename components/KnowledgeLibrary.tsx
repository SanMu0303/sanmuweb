...
import {videoCategories,buildLibrary,filterLibrary,courseCount,courseHref,lessonHref,emptyProgress} from '@/lib/library';
...
export default function KnowledgeLibrary(){
 const [topic,setTopic]=useState<string|null>(null),[type,setType]=useState('all'),[query,setQuery]=useState('');
 ...
 const categories=videoCategories(videos);const selectedTopic=topic!==null&&categories.includes(topic)?topic:null;
 const first=current?.chapters[0]?.lessons[0];const filtered=filterLibrary(buildLibrary(videos,posts.data,courses),selectedTopic,type,query);
 ...
 <>{categories.length>0&&<nav className="learning-topics" aria-label="视频分类"><button aria-pressed={selectedTopic===null} className={selectedTopic===null?'selected':''} onClick={()=>setTopic(null)}>全部分类</button>{categories.map(t=><button key={t} aria-pressed={selectedTopic===t} className={selectedTopic===t?'selected':''} onClick={()=>setTopic(t)}>{t}</button>)}</nav>}</>
 <div className={'learning-layout'+(!outline?' learning-without-course':'')}><div className="learning-main">{current&&<><div className="learning-resume"><span>继续学习</span><p>{progress?.lastWatchedAt?'返回上次学习位置':'暂无观看记录'} <small>进度同步待接入</small></p>{first&&<a href={lessonHref(first)}>开始学习 →</a>}</div><section className="learning-current"><div><small>当前课程 · 示例系列</small><h2>{current.title}</h2><p>{current.chapters[0]?.title}</p></div><div><span>已更新 {count?.published} / {count?.total} 课</span><a href={courseHref(current.id)}>进入课程 →</a></div></section></>}
 {outline&&<CourseDirectorySheet>{outline}</CourseDirectorySheet>}
 ...
 {!filtered.length&&<div className="learning-empty"><p>{query.trim()||selectedTopic!==null||type!=='all'?'暂无匹配资料，试试其他分类或关键词。':'知识库还没有内容，视频发布后会显示在这里。'}</p>{(query.trim()||selectedTopic!==null||type!=='all')&&<button onClick={()=>{setTopic(null);setType('all');setQuery('')}}>清除筛选</button>}</div>}</div>
 </div>{outline&&<aside className="learning-rail">{outline}</aside>}</div></div>;
}
