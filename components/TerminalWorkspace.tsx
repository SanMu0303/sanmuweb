...
export default function TerminalWorkspace() {
  ...
  const path = usePathname() || "/terminal/";
  ...
  return <div className={`terminal-page terminal-page--${theme}`} style={{"--terminal-split":`${split*100}%`} as React.CSSProperties}>
    <header className="terminal-topbar">
      <div className="terminal-brand">
        <MobileNavigationDrawer path={path} theme={theme} session={session} triggerClassName="terminal-site-nav-toggle"/>
        <Link href="/" className="terminal-back"><ArrowLeft size={15}/>返回主站</Link>
        <span className="terminal-brand-divider"/>
        <div><strong>交易终端</strong><small>RESEARCH WORKBENCH</small></div>
      </div>
      <div className="terminal-topbar-actions">
        <span className="terminal-connection"><i aria-hidden="true" className={`terminal-connection-dot terminal-connection-${status}`}/>{connectionText(status)}<span className="terminal-connection-detail">· 仅代表消息接口</span></span>
        <button type="button" className="terminal-top-button terminal-theme-button" aria-pressed={theme==='dark'} onClick={toggleTheme} title={nextThemeLabel} aria-label={nextThemeLabel}>{theme==='soft'?"☾":"☼"}<span>{nextThemeLabel}</span></button>
        <button type="button" className={`terminal-top-button ${audioActive&&config.preferences.sound?"is-on":""}`} onClick={audioActive?toggleAudio:enableAudio} title={audioActive?"关闭提示音":"点击激活提示音"}>{audioActive&&config.preferences.sound?<Volume2 size={15}/>:<VolumeX size={15}/>}<span>{audioActive&&config.preferences.sound?"提示音已开":"提示音"}</span></button>
        <label className="terminal-volume-control" title="调整提示音音量"><Volume2 size={13}/><input type="range" min="0" max="1" step="0.05" value={volumeDraft} onChange={event=>{const value=Number(event.target.value);setVolumeDraft(value);setConfig(current=>({...current,preferences:{...current.preferences,volume:value}}));}} onBlur={event=>void persistVolume(Number(event.currentTarget.value))} aria-label="提示音音量"/><span>{Math.round(volumeDraft*100)}%</span></label>
        <button type="button" className="terminal-top-button terminal-audio-test" onClick={()=>void testAudio()} title="试听提示音"><Headphones size={14}/><span>试听</span></button>
