...
function formatAgo(value:string) { const date = new Date(value); if (!Number.isFinite(date.getTime())) return "时间未知"; return new Intl.DateTimeFormat("zh-CN", {month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).format(date); }
...
function FeedWindow({title,panel,feed,loading,error,paused,onPause,onLatest,newCount,onRefresh,onManage,showManage,sessionRequired,onScroll,scrollRef,soundEnabled,onSoundToggle,filters}:{title:string;panel:Panel;feed:FeedResponse|null;loading:boolean;error:string;paused:boolean;onPause:()=>void;onLatest:()=>void;newCount:number;onRefresh:()=>void;onManage?:()=>void;showManage?:boolean;sessionRequired?:boolean;onScroll?:(event:React.UIEvent<HTMLDivElement>)=>void;scrollRef?:React.Ref<HTMLDivElement>;soundEnabled?:boolean;onSoundToggle?:()=>void;filters?:FeedFilters}) {
  ...
  return <section className={`workbench-window workbench-window-${panel}`} aria-label={title}>
    ...
    {!feed && loading ? <div className="workbench-empty"><RefreshCw className="terminal-spin" size={16}/>正在读取</div> : !feed?.items.length ? <div className="workbench-empty"><Bell size={16}/><span>{status === "unconfigured" ? "尚未接入消息源" : "暂时没有匹配消息"}</span></div> : <div className="workbench-feed-scroll" ref={scrollRef} onScroll={onScroll}>{feed.items.map(item=><FeedItem key={item.id} item={item}/>)}</div>}
  </section>;
}
...
function SmartManager({config,capabilities,onSave}:{config:ConfigResponse["config"];capabilities:Capabilities;onSave:(next:ConfigResponse["config"])=>Promise<void>}) {
...
export default function TerminalWorkspace() {
  ...
  const [market,setMarket]=useState<FeedResponse|null>(null); const [selected,setSelected]=useState<FeedResponse|null>(null); const [smart,setSmart]=useState<{items:TerminalEvent[];sources:SourceState[];accounts:FollowAccount[];fetchedAt:string}|null>(null); const [feedLoading,setFeedLoading]=useState<Record<Panel,boolean>>({market:true,selected:true,smart:true}); const [feedErrors,setFeedErrors]=useState<Record<Panel,string>>({market:"",selected:"",smart:""});
  ...
  const loadPanel=useCallback(async(panel:Panel,reason:RefreshReason="manual")=>{
    ...
    try {
      const response=await request<FeedResponse>(`/api/terminal/${endpoint}${panel==="smart"?"":queryString({...filters,market:filters.market==="全部"?undefined:filters.market,limit:100})}`,{signal:controller.signal});
      ...
      const data={...response,items:mergeEvents(reason==="initial"?[]:latestFeeds.current[panel]?.items||[],items,300) as TerminalEvent[]};
      ...
      lastSuccess.current[panel]=undefined;
      setFeedErrors(value=>({...value,[panel]:errorMessage(error)}));
    } finally {
      if(pendingRequests.current[panel]===controller){pendingRequests.current[panel]=null;setFeedLoading(value=>({...value,[panel]:false}));}
    }
  },[renderFeed]);
  ...
  return <div className="terminal-page" style={{"--terminal-split":`${split*100}%`} as React.CSSProperties}>
    ...
    <main className="terminal-main" aria-label="行情研究终端"><aside className={`terminal-monitor-area ${activeView!=="market"&&activeView!=="selected"&&activeView!=="smart"?"mobile-hidden":""}`}>
      ...
      {manager==="smart"&&<SmartManager config={config} capabilities={capabilities} onSave={updateConfig}/>}
