...
export default function Feed({articles,home=false,watchItems=[],onPublished}:{articles:Post[];home?:boolean;watchItems?:WatchItem[];onPublished?:()=>void}){
 ...
 // Homepage member cards all use the same compact gate. Previously the first
 // restricted result was intentionally expanded while later results were
 // compact, which made two otherwise identical prompts differ in height.
 const renderedStream=stream.map(p=><PostCard key={p.id} post={p} compact={home} memberGateCompact={home&&p.locked?true:undefined} signedIn={session.data?.signedIn} isAdmin={home&&!!session.data?.isAdmin} onWatchSaved={onPublished}/>);
