...
export default function Membership(){
 ...
 return <div className="membership"><div className="page-heading"><div><div className="eyebrow">MEMBERSHIP</div><h1>会员中心</h1><p>持续研究，随时回看。</p></div></div>
  <section className="member-status">
   {user.signedIn?<><h2>{user.nickname}，你好</h2><MemberStatusSummary user={user}/></>:<><span className="member-badge none">访客</span><h2 className="member-card-title">登录后查看会员状态</h2><p>注册账号后可以保存个人资料，会员资格由管理员开通。</p></>}
   ...
   {!hasAccess&&<div className="notice">{!user.signedIn?'请先登录账号。需要开通会员时，请联系管理员。':status==='expired'?'会员已到期。需要续期时，请联系管理员；公开内容仍可阅读。':status==='revoked'?'会员资格已撤销。如需重新开通，请联系管理员；公开内容仍可阅读。':'当前账号尚未开通会员。请联系管理员开通，注册账号不会自动解锁会员内容。'}</div>}
   <div className="actions">{user.signedIn?<><Link className="button" href="/knowledge/">{hasAccess?'进入知识库':'浏览公开课程'}</Link><Link className="button secondary" href="/profile/">个人中心</Link>{user.isAdmin&&<Link className="button secondary" href="/admin/members/">管理用户与会员</Link>}</>:<><Link className="button" href="/login/">登录账号</Link><Link className="button secondary" href="/register/">注册账号</Link><Link href="/knowledge/">先读公开课程 ↗</Link></>}</div>
  </section>
 </div>;
}
