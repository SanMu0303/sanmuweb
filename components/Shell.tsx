...
function NavigationContent({path,theme}:{path:string;theme:SiteTheme}){const session=useResource<{isAdmin:boolean;signedIn:boolean;nickname:string;avatarUrl:string}>('/api/session');return <>
 <div className="sidebar-heading"><strong>研究工作台</strong></div>
 <nav aria-label="研究栏目">{nav.map(([href,label,Icon])=>{const active=isNavActive(path,href);return <Link key={href} href={href} className={active?'active':''} aria-current={active?'page':undefined}><Icon size={16}/><span>{label}</span></Link>})}</nav>
 <div className="side-note"><span className="eyebrow">THE PROCESS MATTERS</span><p>观察。等待。执行。<br/>让每一次判断都有迹可循。</p><i/></div>
 <div className="sidebar-account-links">
  <Link className="member-nav" href="/membership/"><UserRound size={16}/><span>会员中心</span><ArrowUpRight size={14}/></Link>
  {session.data?.signedIn&&<Link href="/profile/" className={'admin-entry '+(isNavActive(path,'/profile/')?'active':'')} aria-current={isNavActive(path,'/profile/')?'page':undefined}><UserRound size={16}/><span>个人中心</span><ArrowUpRight size={13}/></Link>}
  {session.data?.isAdmin&&<Link href="/admin/" className={'admin-entry '+(isNavActive(path,'/admin/')?'active':'')} aria-current={isNavActive(path,'/admin/')?'page':undefined}><NotebookPen size={16}/><span>内容管理</span><ArrowUpRight size={13}/></Link>}
 </div>
 <div className="side-bottom"><span className="site-live-dot"/>{theme==='dark'?'暗色界面已启用':'柔和亮面已启用'}</div>
 </>}
