...
try{
 const room=(await repo.list('watch_items')).filter(w=>w.isWeeklyFocus).length<3;
 const first=await repo.saveWithWatch(post,0,owner,{revision:0,summary:'公开判断',invalidation:'跌破区间',weeklyFocus:room});
 ...
 assert.equal(watch.isWeeklyFocus,room);
 const history=await repo.history(key);assert.deepEqual(history.map(h=>h.revision),[2,1]);assert.equal(history[1].document.thesis,'公开判断');assert.equal(history[0].document.thesis,'结构失效');assert.equal(history[0].document.isWeeklyFocus,room);
 ...
 console.log('History versions 1/2 retained; focus selection preserved; focus slot available:',room);
 ...
 await client.request('/rest/v1/watch_history?'+new URLSearchParams({symbol:'eq.'+key}),{method:'DELETE'});
