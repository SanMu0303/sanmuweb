...
export async function request<T>(path:string,init?:RequestInit):Promise<T>{
 ...
 const membershipWrite=path.startsWith('/api/admin/members/')&&init?.method==='PUT';
 try{
  const response=await fetch(path,{cache:'no-store',...init,signal:init?.signal||AbortSignal.timeout(45000)});
  const data=await response.json().catch(()=>null);
  if(!response.ok)throw Object.assign(new Error(data?.error||(response.status===401?'登录已过期，请重新登录。':`请求失败（${response.status}），请稍后检查。`)),{status:response.status});
  if(data===null)throw new Error(authRequest?'登录服务返回异常，请稍后重试。':'服务器返回异常，请先检查内容是否已保存。');
  return data;
 }catch(error){if(error instanceof DOMException&&['TimeoutError','AbortError'].includes(error.name))throw new Error(membershipWrite?'保存结果尚未确认，请刷新会员列表核对后再操作。':codeRequest?'请求超时。若已收到验证码，可直接填写后提交；否则稍后重试。':authRequest?'请求超时，请稍后重试。如果刚才在重置密码，可返回登录页尝试使用新密码。':'请求超时，保存结果尚未确认。请在新窗口检查最新内容，确认未发布后再重试；当前正文已保留。');if(error instanceof TypeError)throw new Error(membershipWrite?'网络连接中断，保存结果尚未确认，请刷新会员列表核对后再操作。':authRequest?'网络连接中断，请检查网络后重试。':'网络连接中断，保存结果尚未确认。请先检查最新内容再重试；当前正文已保留。');throw error}
}
