...
begin
 ...
 'isWeeklyFocus',coalesce((p_sync->>'weeklyFocus')::boolean,(old_document->>'isWeeklyFocus')::boolean,false),'articleSlug',p_document->>'slug');
...
grant select,insert,delete on public.watch_history to service_role;
-- Seed only the currently available snapshot; older overwritten versions cannot be reconstructed.
insert into public.watch_history(symbol,revision,document)
select symbol,revision,document from public.watch_items on conflict do nothing;
create or replace function public.record_watch_history() returns trigger
language plpgsql security invoker set search_path=public,pg_temp as $$
begin
 if TG_OP='UPDATE' then
  insert into watch_history(symbol,revision,document) values(OLD.symbol,OLD.revision,OLD.document) on conflict do nothing;
 end if;
 insert into watch_history(symbol,revision,document) values(NEW.symbol,NEW.revision,NEW.document) on conflict do nothing;
 return NEW;
end $$;
revoke all on function public.record_watch_history() from public,anon,authenticated;
grant execute on function public.record_watch_history() to service_role;
drop trigger if exists watch_history_snapshot on public.watch_items;
create trigger watch_history_snapshot after insert or update on public.watch_items for each row execute function public.record_watch_history();
commit;
