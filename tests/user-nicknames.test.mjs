...
const request=()=>new Request('https://site.test',{headers:{cookie:'research_access=verified-token'}});
...
test('canonical persistent nickname wins over mutable Auth metadata and needs a verified user',async()=>{
 ...
 const auth=createAuth(env,async(url,options)=>{
  ...
  if(url.endsWith('/user')){assert.equal(options.headers.Authorization,'Bearer verified-token');return Response.json({...user,user_metadata:{nickname:untrustedMetadataName}})}
...
test('failed Auth verification never reaches service-role nickname RPC',async()=>{
 for(const bad of [{...user,id:null},{...user,email_confirmed_at:null},{...user,email:null}]){
  let calls=0;const auth=createAuth(env,async url=>{calls++;assert.ok(url.endsWith('/user'));return Response.json(bad)});
  assert.equal((await auth.identify(request())).signedIn,false);assert.equal(calls,1);
  await assert.rejects(auth.updateProfile(request(),'昵称'),{status:401});assert.equal(calls,2);
 }
});
...
test('legacy service-role keys are used only for nickname RPC, never replacing verified Auth bearer',async()=>{
 ...
 const auth=createAuth(legacyEnv,async(url,options)=>{
  if(url.endsWith('/user')){assert.equal(options.headers.Authorization,'Bearer verified-token');return Response.json(user)}
...
test('nickname pool exhaustion returns clear 503 without a shared fallback or sign-in cookie',async()=>{
 const auth=createAuth(env,async url=>{
  if(url.includes('/token?'))return Response.json({access_token:'verified-token',expires_in:3600});
...
test('unavailable nickname storage never generates an ephemeral fallback',async()=>{
 const auth=createAuth(env,async url=>url.endsWith('/user')?Response.json(user):Response.json({code:'XX000',message:'private details'},{status:500}));
...
test('profile changes reject another user avatar before the privileged RPC',async()=>{
 let calls=0;const auth=createAuth(env,async url=>{calls++;assert.ok(url.endsWith('/user'));return Response.json(user)});
 await assert.rejects(auth.updateProfile(request(),'新昵称','avatars/'+otherId+'/cccccccc-cccc-4ccc-8ccc-cccccccccccc.jpg'),{status:400});assert.equal(calls,1);
});

test('database nickname conflicts remain 409 and cannot fall back to unprotected Auth metadata',async()=>{
 const calls=[];const auth=createAuth(env,async(url,options)=>{
  calls.push({url,method:options.method});
  return url.endsWith('/user')?Response.json(user):Response.json({code:'PT409',message:'private details'},{status:409});
 });
 await assert.rejects(auth.updateProfile(request(),'别人已使用的昵称'),error=>error.status===409&&error.message==='该昵称已被使用，请更换昵称');
 assert.deepEqual(calls.map(c=>c.method),['GET','POST']);assert.ok(calls[1].url.endsWith('/set_user_nickname'));
});
