...
test('login passes email/password intact and sets HttpOnly cookie; session recognizes admin',async()=>{
 ...
 const r=await api(req({},JSON.stringify({email:user.email,password:'test-password'})));
 assert.equal(r.status,200);assert.deepEqual(received,{email:user.email,password:'test-password'});
