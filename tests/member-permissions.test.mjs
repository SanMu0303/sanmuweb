...
test('legacy excerpts are never reused as member execution previews',async()=>{
  ...
  assert.equal(body.title,'标的 · 研究更新');
...
test('locked projections expose only fixed safe placeholder rows for the member mask',async()=>{
  ...
  const post=projectPost({slug:'masked',title:'私有标题',publicTitle:'ETH · 研究更新',excerpt:'公开摘要',publishedAt:'2026-09-20',sections:[{heading:'正文',text:secret}],access:'member',images:[]},false);
  assert.equal(post.locked,true);
  assert.equal(post.access,'member_required');
  assert.equal(post.publicTitle,'ETH · 研究更新');
  assert.equal(post.title,'ETH · 研究更新');
  assert.deepEqual(post.maskedLines,['••••••••••••••••••••••','••••••••••••••••','••••••••••••••••••••••••','••••••••••••••••••','••••••••••••••••••••••']);
  assert.equal(JSON.stringify(post).includes(secret),false);
  assert.deepEqual(post.memberPrompt,{label:'会员专享',title:'会员专享 · 完整研究记录',description:'',cta:'开通会员'});
  ...
  assert.deepEqual(active.memberPrompt,{label:'会员专享',title:'会员专享 · 完整观察记录',description:'观察结束后免费公开',cta:'开通会员'});
